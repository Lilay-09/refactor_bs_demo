...
class TransactionService
{
    ...
    public function getDeliveryPackagesV1(Request $req,$type,$user){
        ...
        $qP = Package::query()
            ...
            ->join('users as d','d.id','packages.driver_id')
            ->join('tracking_statuses as ts','ts.id','packages.status_id')
            ->join('users as m','m.id','packages.merchant_id')
            ->whereIn('packages.status_id',[9,19])
            ->select([
                'packages.extra_charge','packages.additional_fee','packages.remarks','packages.cod','packages.price','packages.price_khr','d.phone as driver_phone','packages.taxi_fee','packages.payer','packages.delivery_fee','packages.assign_driver_datetime',
                'packages.merchant_total','m.username as merchant_name','m.phone as merchant_phone','d.username as driver_name','packages.status_id','packages.id as package_id','d.id as driver_id',
                'packages.qr_code','ts.name as status_code','packages.delivered_datetime','packages.failed_datetime','packages.zone_code','packages.receiver_phone','packages.delivery_type','packages.zone_name',
                'packages.receiver_address','packages.driver_cod_usd','packages.driver_cod_khr','packages.other_fee','packages.original_driver_cod_usd','packages.original_driver_cod_khr','packages.method'
            ]);
            if ($pmtStatusId == 1) { // Unpaid
                $qP->whereNotExists(function ($sub) use($type) {
                    $sub->select(DB::raw(1))
        ...
        if(!$search){
            if($startDate && $endDate){
                ...
                $qP->where(function ($q) use ($startDate, $endDate) {
                    ...
                    // Check for status_id = 9, delivered_datetime should be within the date range
                    $q->where(function ($q) use ($startDateTime, $endDateTime) {
                        $q->where('packages.status_id', 9)
                        ->whereRaw('packages.delivered_datetime >= ? AND packages.delivered_datetime <= ?', [$startDateTime, $endDateTime]);
                    })
                    // Check for status_id = 19, failed_datetime should be within the date range
                    ->orWhere(function ($q) use ($startDateTime, $endDateTime) {
                        $q->where('packages.status_id', 19)
                        ->whereRaw('packages.failed_datetime >= ? AND packages.failed_datetime <= ?', [$startDateTime, $endDateTime]);
                    });
                });
            }
            ...
            if($driverId || $merchantId){
                if($type == 'driver') {
                    $qP->where('packages.driver_id',$driverId);
                }
                else {
                    $qP->where('packages.merchant_id',$merchantId);
                }
            }
        }

        else{
            $qP->where('packages.qr_code',$search);
        }
        ...
        $clbMapper = function($package) use($statusKey,$type){
            $cod = $package->cod;
            $package->cod = $cod ? 'Yes' : 'No';
            $package->paymentPackages = $package->paymentPackages
                ->filter(fn($pp) => !$pp->is_deleted && optional($pp->payment)->payer_id == $package->driver_id);
    ...
    public function disburesementBulkV1(Request $req,$user,$type){
        ...
        if (!empty($currencyConflictInfo)) {
            ...
            return DataResponse::Duplicated("These packages already partially paid with {$currency} for {$type}(s): {$users}.");
        }
        // return DataResponse::JsonResult($insertPayout);
        try {
            DB::beginTransaction();
    ...
    private function preparePayIn(array $pkgIds, string $type, string $payingCurrency, object $validPkg, int $mId, object $payInPkgs)
    {
        $result = [
            'fullyPaidInfo'       => [],
            'currencyConflictInfo'=> [],
    ...
    //         if (($usdDue == 0) && ($khrDue == 0) && ($payment->payment_status_id == PaymentStatus::REQUESTED->value)){
    //             $result['fullyPaidInfo'][] = [
    //                 'package_id'    => $pkgId,
    //                 "{$type}_id"    => $mId,
    //                 "{$type}_name"  => $validPkg->data["{$type}_name"] ?? $mId,
    ...
    //                 if (($khrDue == $validKhrAmt) || ($validKhrAmt == 0)) {
    //                     $result['allKHRReceived'] = true;
    //                 }
    //             } else {
    //                 $result['allUSDReceived'] = false;
    ...
    //                 if (($usdDue == $validUsdAmt) || ($validUsdAmt == 0)) {
    //                     $result['allUSDReceived'] = true;
    //                 }
    //             } else {
    //                 $result['allKHRReceived'] = false;
    ...
    private function preparePayout(array $pkgIds, string $type, string $payingCurrency, object $validPkg, int $mId, object $disbursementPkg): array
    {
        $result = [
            'fullyPaidInfo'       => [],
            'currencyConflictInfo'=> [],
    ...
    //         $result['targetId'] = $disbursement->id;
    //         // Already fully paid
    //         if (($usdDue == 0) && ($khrDue == 0) && ($disbursement->payment_status_id == PaymentStatus::REQUESTED->value)) {
    //             $result['fullyPaidInfo'][] = [
    //                 'package_id'    => $pkgId,
    //                 "{$type}_id"    => $mId,
    //                 "{$type}_name"  => $validPkg->data["{$type}_name"] ?? $mId,
    ...
    public function validPackagesV1($packageIds,$driverOrMerchantId,$type){
        ...
        $usd = max(0, $obj->total_due_amount_usd);
        $khr = max(0, $obj->total_due_amount_khr);
        Helper::deductAmountBase($usd,$khr,($obj->total_fees + $obj->total_taxi_fee));
        ...
        return DataResponse::JsonRaw([
            ...
            'failed_with_fee_count' => $obj->failed_with_fee_count,
            'total_cod' => $obj->total_cod,
            'total_due_amount_khr' => $khr,//$obj->total_due_amount_khr,
            'total_due_amount_usd' => $usd,//$obj->total_due_amount_usd,
            'total_amount' => $obj->total_amount,
            'delivered_package_count' => $obj->delivered_package_count,
        ]);
    }
    ...
    public function validBulkPackagesV1($packages,$packageIds,$driverOrMerchantId,$type){
        ...
        foreach($packageIds as $index=>$id){
            ...
            // if($package->driver_cod_usd == 0 && $package->driver_cod_khr == 0){
                // if (!isset($byMerchant[$package->merchant_id])) {
                //     $byMerchant[$package->merchant_id] = [
                //         'qr' => [],
                //         'id' => [],
                ...
                // $fees = ($package->payer == 'sender') ? ($package->delivery_fee + $package->other_fee):0;
                // $byMerchant[$package->merchant_id]['driver_cod_usd'] += $driverCodUsd;
                // $byMerchant[$package->merchant_id]['driver_cod_khr'] += $driverCodKhr;
                // $byMerchant[$package->merchant_id]['qr'][] = $package->qr_code;
                // $byMerchant[$package->merchant_id]['fees'] += number_format($fees,2,'.','');
                // $byMerchant[$package->merchant_id]['taxi_fee'] += $package->taxi_fee;
            // }
        ...
        // if($paymentType == 'disbursement') $obj->total_due_amount = abs($obj->total_due_amount);
        // if(!empty($byMerchant)){
        //     // Log::error(json_encode($byMerchant));
        //     $filtered = collect($byMerchant)->filter(function ($item) {
        //         return $item['merchant_id'] == 663;
        //     });
        //     if(!empty($filtered)){
    ...
    public function getBalanceV1(Request $req,$user,$type='driver'){
        ...
        $qP = User::from('users as d')
            ...
            ->select([
                ...
                'p.driver_cod_usd','p.price_khr','p.id as package_id','p.method','p.currency'
            ]);
        ...
        ->map(function ($packages, $key) use (&$totalToBePaidUsd,&$totalToBePaidKhr,&$totalPackages, &$totalAmount, &$totalAmountKhr, &$totalDriverCodUsd, &$totalDriverCodKhr, &$totalOtherFee,&$totalBaseFee, &$totalTaxiFee,&$summaryTotals) {
            ...
            ->flatMap(function ($group, $method) {
                $usd = $group->sum('driver_cod_usd');
                $khr = $group->sum('driver_cod_khr');
                ...
                if ($usd > 0) {
                    $result->push([
                        'method' => $method,
                        'currency_code' => 'USD',
                        'total_amount' => number_format($usd, 2, '.', ''),
                    ]);
                }
                ...
                if ($khr > 0) {
                    $result->push([
                        'method' => $method,
                        'currency_code' => 'KHR',
                        'total_amount' => number_format($khr, 2, '.', ''),
                    ]);
                }
            ...
            return [
                ...
                'base_fee' => Helper::getNumber($deliveryFee,2,true),
                'other_fee' => Helper::getNumber($otherFee,2,true),
                'taxi_fee' => Helper::getNumber($taxiFee,2,true),
                'to_be_paid_usd' => Helper::getNumber($tobePaidUsd,2,true),
                'to_be_paid_khr' => Helper::getNumber($tobePaidKhr,0,true),
        ...
        return DataResponse::Pagination(collect($groupData),$req,__('messages.Get List'),[
            ...
            'summary_collected' => $finalSummary
        ]);
    }
