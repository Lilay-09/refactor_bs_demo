...
class PriceListController extends Controller
{
    ...
    private function getZoneNames($zones,$identifier,$priceListId){
        $zoneNames = [];
        foreach($zones as $z){
            if($z->identifier == $identifier && $z->price_list_id == $priceListId){
                $zoneNames[] = $z->zone->zone_name;
            }
        }
        return $zoneNames;
    }
    ...
    public function getPriceZones(Request $req){
        ...
        $priceZones = $pZ->with(['priceList:id,delivery_type'])->selectRaw('price_list_id,identifier')
            ...
            ->get()
            ->toArray(); // Convert to array for safety
            // var_dump($priceZones);
        $arrObj = [
            [
        ...
        foreach ($arrObj as &$arr) {
            if ($arr['key'] === 'below' || $arr['key'] === 'above') {
                ...
                foreach ($priceZones as &$z) {
                    ...
                    // To track the added delivery_type combinations for each identifier
                    $addedCombination = [];
